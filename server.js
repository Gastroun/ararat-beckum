...
app.post('/api/create-stripe-checkout', async (req, res) => {
  try {
    ...
    const appFee = Math.round(((serviceFee||0) + (subtotal * 0.05)) * 100) + stripeFee;
...
cron.schedule('0 22 * * 0', async () => {
  try {
    ...
    // ── PDF 1: Wochenbericht (Kennzahlen + Kundenliste + Bar-Rechnung) ────
    const berichtPdf = await generatePdf(doc => {
      pdfColorBox(doc, `Wochenbericht KW ${kw} / ${now.getFullYear()}`, `Ararat Grill Beckum  ·  ${vonBis}`, '#c0392b');
      pdfKacheln(doc, [
    ...
    const rechnungPdf = await generatePdf(doc => {
      ...
      doc.fontSize(10).font('Helvetica').fillColor('#555').text('Nordwall 45', 310, addrY+30).text('59269 Beckum', 310, addrY+44);
      doc.y = addrY + 80;
      ...
      doc.font('Helvetica').fontSize(9).fillColor('#888').text(`${pdfFmt(PDF_SV)} × ${orders.length} Bestellungen (KW ${kw})`);
      const sfY = doc.y - 32;
      ...
      doc.fontSize(8).fillColor('#aaa').text(`Abed Rachman Falah · ${rechnungNr} · KW ${kw}/${now.getFullYear()}`, 50, 780, { width:W, align:'center' });
    });
  ...
  <div style="padding:24px 28px">
    ...
    <p style="font-size:11px;color:#aaa;margin-top:8px">Anbei der Wochenbericht mit Kundenliste${barOrders.length > 0 ? ' und Bar-Rechnung' : ''}.</p>
        ...
        attachments: [{ filename: `KW${kw}_${now.getFullYear()}_Ararat_Wochenbericht.pdf`, content: berichtPdf.toString('base64') }],
      });
    }
    ...
    if (process.env.OWNER_EMAIL) {
      await getResend()?.emails.send({
        ...
        html: `<p style="font-family:Arial,sans-serif;color:#555">Anbei Rechnung <b>${rechnungNr}</b> und Wochenbericht KW ${kw} / ${now.getFullYear()} für Ararat Grill Beckum.</p>
               <p style="font-family:Arial,sans-serif;color:#555"><b>Zeitraum:</b> ${vonBis}<br><b>Verdienst:</b> ${meinBetrag.toFixed(2).replace('.',',')} €${barOrders.length > 0 ? `<br><b>Bar-Rechnung:</b> ${barBetrag.toFixed(2).replace('.',',')} € (${barOrders.length} Barzahlungen)` : ''}</p>`,
        attachments: [
...
cron.schedule('0 22 * * *', async () => {
  ...
  <div style="padding:24px 28px">
    ...
    <p style="font-size:11px;color:#aaa;margin-top:8px">Anbei der Monatsbericht mit Kundenliste${barOrdersM.length > 0 ? ' und Bar-Rechnung' : ''}.</p>
        ...
        attachments: [{ filename: `${monat.replace(' ','_')}_Ararat_Monatsbericht.pdf`, content: monatsPdf.toString('base64') }],
      });
    }
    ...
    if (process.env.OWNER_EMAIL) {
      await getResend()?.emails.send({
        from: process.env.EMAIL_FROM || 'system@ararat-grill.com',
        to:   process.env.OWNER_EMAIL,
        subject: `🧾 ${rechnungNr} + Monatsbericht ${monat} · Ararat Grill Beckum`,
        html: `<p style="font-family:Arial,sans-serif;color:#555">Anbei Rechnung <b>${rechnungNr}</b> und Monatsbericht <b>${monat}</b> für Ararat Grill Beckum.</p>
        ...
        attachments: [
          ...
          { filename: `${monat.replace(' ','_')}_Ararat_Monatsbericht.pdf`,              content: monatsPdf.toString('base64') },
        ],
      });
    }
